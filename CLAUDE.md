...
- `CCNetworkGroupService` - Network groups, members, peers, WireGuard configs. Gotchas learned against the live v4 API:
  ...
  - **The v4 NG backend intermittently answers 500 to writes** ("There was an internal server error.", text/plain) while accepting the byte-identical body seconds later — proven server-side (2026-07, reported to Clever Cloud, CTO investigating): failures are per-request random (observed 500,500,202 for identical bodies on one connection within 5s), GETs always succeed. All three NG writes ride over it:
    - member POST → `retryingOnServerError` (2 retries, 2s pause, 5xx only). Safe: replaying the same member id is an **upsert** on the server.
    - NG creation → client-generated lowercase `ng_<uuid>` id + `ownerId` sent in the body (clever-tools style, the API honors client ids) makes the POST idempotent → same retry; the created group is then resolved **by its known id** with a poll (500ms/~30), not by list-diff/name-match.
    - external-peers POST → `createExternalPeerRecovering`: its id is server-generated so a blind replay could duplicate the peer; on 5xx the app first looks the peer up **by publicKey** (unique per attach) and adopts it if the POST actually landed, else replays (≤2×).
    - DELETE paths have no retry yet — add one if the flakiness bites there (deletes are idempotent, a 404 on replay is tolerable).
  - Network groups work in the **personal space** too (`ownerId = user_…`) — an earlier "can't create NGs in personal space" symptom was just the intermittent 500 above.
  - The API **ignores the requested `networkIp`** (every group comes back `10.101.0.0/16`), hence no CIDR field in `CreateNetworkGroupView`.
  - Member `domainName` (`<resourceId>.m.<ngId>.cc-ng.cloud`) and peer NG-internal IP (`endpoint.ngTerm.host` for CleverPeers / `endpoint.ngIp` for external peers) are decoded and shown in the Members/Peers rows.
...
- `NetworkGroupDetailView` - Network group detail (Overview / Members / Peers tabs). The Overview tab hosts the **"VPN on this device" card**: a Toggle when the persisted VPN profile belongs to this group (`tunnel.configuredNetworkGroupId == networkGroup.id` → `startSaved()`/`disconnect()`), an "Attach this device instead" button when it belongs to another group, or an "Attach this device" shortcut when no profile exists. Removing the device's external peer, or deleting the whole group, calls `tunnel.tearDown()` so the VPN profile is removed from iOS Settings along with the dead peer. **Two distinct peer-creation flows** (Peers tab "+" menu → "This device…" / "Another device…"), both built on the shared `WireGuardProvisioning.provision()` pipeline (local Curve25519 keygen → `createExternalPeer` → fetch `.conf` → inject private key):
  - `AttachDeviceView` — THIS device: name pre-filled from `UIDevice.current.name`, then peer creation + **mandatory VPN profile install & connect** (`tunnel.connect` — `saveToPreferences` triggers the iOS VPN-authorization prompt). The private key goes straight to the keychain and is never displayed. Without this step the toggle card would never activate, since `connect()` is what persists the profile. **Re-attaching to a different group replaces the single profile and overwrites the keychain private key**, permanently killing the previous peer — the view warns with a destructive confirmation and, after the new attach succeeds, deletes the orphaned peer from its old group (`deleteExternalPeerCascading`, best-effort; requires the `organizationId` persisted in `providerConfiguration`, absent on pre-2026-07-14 profiles).
  - `WireGuardConfigView` — ANOTHER device: shows the assembled `.conf` as text + QR + expiring-clipboard copy. Never touches this device's VPN profile.
