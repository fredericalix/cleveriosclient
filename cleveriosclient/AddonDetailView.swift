...
struct AddonDetailView: View {
    ...
    @State private var metricsPeriod = "PT1H"
    @State private var isLoadingMetrics = false
    @State private var metricsError: String?
    @State private var realtimeMetrics: CCAddonRealtimeMetrics?
    @State private var metricsTimeSeries: [String: [CCAddonTimeSeriesData]] = [:]
    ...
    // MARK: - Tab 3: Metrics (Placeholder)
    
    private var metricsTab: some View {
        VStack(spacing: 30) {
            // Metrics placeholder - API not fully functional yet
            VStack(spacing: 40) {
                Spacer()
                ...
                // Title
                Text("Metrics Under Development")
                    .font(.title2)
                    .fontWeight(.bold)
                
                // Description
                VStack(spacing: 8) {
                    Text("Metrics API integration is in progress.")
                        .font(.subheadline)
                        .foregroundColor(.secondary)
                    
                    Text("Token retrieval works, but the v4 API endpoint is not accessible yet.")
                        .font(.caption)
                        .foregroundColor(.secondary)
                        .multilineTextAlignment(.center)
            ...
            if isLoadingMetrics {
                ...
                ScrollView {
                    VStack(spacing: 20) {
                        ...
                        if let realtime = realtimeMetrics {
                            metricsCardsGrid(metrics: realtime)
                        }
                        
                        // Time series charts
                        if !metricsTimeSeries.isEmpty {
    ...
    private var metricsHeader: some View {
        VStack(spacing: 12) {
            HStack {
                ...
                // Period selector
                Picker("Period", selection: $metricsPeriod) {
                    Text("1 hour").tag("PT1H")
                    Text("24 hours").tag("PT24H")
                    Text("7 days").tag("P7D")
                    Text("30 days").tag("P30D")
                }
                .pickerStyle(.segmented)
                .frame(maxWidth: 280)
                .onChange(of: metricsPeriod) { _, _ in
                    loadMetrics()
                }
            }
            
            // Show last updated time based on current time
            HStack {
    ...
    private func metricsCardsGrid(metrics: CCAddonRealtimeMetrics) -> some View {
        VStack(spacing: 16) {
            // Provider-specific metrics
            switch addon.provider.id.lowercased() {
            case "postgresql-addon", "postgres", "mysql-addon":
                // Database metrics
                HStack(spacing: 12) {
                    if let cpu = metrics.cpu {
                        MetricCard(
                            title: "CPU Usage",
                            value: String(format: "%.1f%%", cpu.current),
                            icon: "cpu",
                            color: .blue,
                            trend: cpu.average
                        )
                    ...
                    if let memory = metrics.memory {
                        MetricCard(
                            title: "Memory",
                            value: formatMemory(memory.current),
                            icon: "memorychip",
                            color: .purple,
                            trend: memory.average
                        )
                    }
                }
                
                HStack(spacing: 12) {
                    if let connections = metrics.connections {
                        MetricCard(
                            title: "Connections",
                            value: String(format: "%.0f", connections.current),
                            icon: "network",
                            color: .green,
                            trend: connections.average
                        )
                    }
                    
                    if let queries = metrics.queries {
                        MetricCard(
                            title: "Queries/s",
                            value: String(format: "%.1f", queries.current),
                            icon: "chart.line.uptrend.xyaxis",
                            color: .orange,
                            trend: queries.average
                        )
                    }
                }
    ...
    private var metricsChartsSection: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            // CPU Chart
            if let cpuData = metricsTimeSeries["cpu"], !cpuData.isEmpty {
                MetricChart(
                    title: "CPU Usage",
                    data: cpuData,
                    color: .blue,
            ...
            // Memory Chart
            if let memoryData = metricsTimeSeries["memory"], !memoryData.isEmpty {
                MetricChart(
                    title: "Memory Usage",
                    data: memoryData,
                    color: .purple,
            ...
            // Connections Chart (for databases)
            if let connectionsData = metricsTimeSeries["connections"], !connectionsData.isEmpty {
                MetricChart(
                    title: "Active Connections",
            ...
            // Queries Chart (for databases)
            if let queriesData = metricsTimeSeries["queries"], !queriesData.isEmpty {
                MetricChart(
    ...
    private func loadMetrics() {
        ...
        viewModel.cleverCloudSDK.addons.getAddonMetrics(
            addonId: addon.realId ?? addon.id,
            organizationId: organizationId,
        ...
        .sink(
            ...
            receiveValue: { metrics in
                print("✅ [AddonDetailView] Successfully loaded metrics")
                
                // Convert to realtime metrics format
                self.realtimeMetrics = CCAddonRealtimeMetrics(
                    cpu: metrics.cpuUsage != nil ? CCAddonRealtimeMetrics.MetricSnapshot(
                        current: metrics.cpuUsage!,
                        average: metrics.cpuUsage!,
                        peak: metrics.cpuUsage!,
                        unit: "%"
                    ) : nil,
                    memory: metrics.memoryUsage != nil ? CCAddonRealtimeMetrics.MetricSnapshot(
                        current: metrics.memoryUsage!,
                        average: metrics.memoryUsage!,
                        peak: metrics.memoryUsage!,
                        unit: "MB"
                    ) : nil,
                    connections: metrics.connections != nil ? CCAddonRealtimeMetrics.MetricSnapshot(
                        current: Double(metrics.connections!),
                        average: Double(metrics.connections!),
                        peak: Double(metrics.connections!),
                        unit: "connections"
                    ) : nil,
                    queries: metrics.queries != nil ? CCAddonRealtimeMetrics.MetricSnapshot(
                        current: Double(metrics.queries!),
                        average: Double(metrics.queries!),
                        peak: Double(metrics.queries!),
                        unit: "q/s"
                    ) : nil,
                    storage: nil,
                    networkIn: nil,
    ...
    private func loadTimeSeriesData() {
        // Define which metrics to fetch based on add-on type
        let metricsToFetch: [String] = {
            switch addon.provider.id {
            case "postgresql", "mysql", "mongodb":
                return ["cpu", "memory", "connections", "queries"]
            case "redis":
        ...
        for metric in metricsToFetch {
            viewModel.cleverCloudSDK.addons.getAddonTimeSeries(
                addonId: addon.realId ?? addon.id,
                organizationId: organizationId,
                metric: metric,
                interval: metricsPeriod == "PT1H" ? "PT1M" : "PT5M",
                from: from,
                to: to
            )
            .receive(on: DispatchQueue.main)
            .sink(
                receiveCompletion: { completion in
                    if case .failure(let error) = completion {
                        print("❌ Failed to load \(metric) time series: \(error)")
                    }
                },
                receiveValue: { dataPoints in
                    self.metricsTimeSeries[metric] = dataPoints.map { point in
                        CCAddonTimeSeriesData(timestamp: point.timestamp, value: point.value)
                    }
                }
            )
            .store(in: &cancellables)
